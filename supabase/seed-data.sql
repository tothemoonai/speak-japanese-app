...
-- =====================================================
-- 4. 插入一些模拟用户数据（需要在 auth.users 中先创建）
-- 注意：这些需要通过真实的用户注册流程创建
-- 这里只是示例数据

-- 假设已经有一些用户注册，添加用户配置信息
-- UPDATE public.users SET nickname = '测试用户1', level = 'beginner' WHERE email = 'test1@example.com';
-- UPDATE public.users SET nickname = '测试用户2', level = 'intermediate' WHERE email = 'test2@example.com';

-- =====================================================
-- 5. 插入分享示例数据
-- =====================================================

INSERT INTO public.shares (share_code, user_id, share_type, target_id, title, description, image_url, click_count, expires_at) VALUES
('PRAC001', (SELECT id FROM public.users LIMIT 1), 'practice', 1, '我的练习成果', '刚刚完成了第1课的练习，得分95分！', 'https://images.unsplash.com/photo-1434030216411-1b770f9c9cc1?w=600', 0, NOW() + INTERVAL '30 days'),
('COURSE01', (SELECT id FROM public.users LIMIT 1), 'course', 1, '推荐课程', '这门课程非常适合初学者', 'https://images.unsplash.com/photo-1526245478-1bac7fa03135?w=600', 0, NOW() + INTERVAL '7 days');
...
SELECT
  ...
  c.theme,
  c.total_sentences,
  COUNT(DISTINCT s.id) as actual_sentences,
  (SELECT COUNT(*) FROM public.practice_records pr WHERE pr.course_id = c.id AND pr.completed_at IS NOT NULL) as practice_count
FROM public.courses c
LEFT JOIN public.sentences s ON s.course_id = c.id
GROUP BY c.id;
...
SELECT
  ...
  u.nickname,
  u.level,
  COUNT(DISTINCT pr.course_id) as courses_practiced,
  SUM(pr.time_spent) as total_time_spent,
  AVG(pr.total_score) as average_score
FROM public.users u
LEFT JOIN public.practice_records pr ON pr.user_id = u.id AND pr.completed_at IS NOT NULL
GROUP BY u.id;
...
-- 添加注释
COMMENT ON TABLE public.characters IS '对话角色表';
COMMENT ON TABLE public.practice_records IS '用户练习记录';
COMMENT ON TABLE public.practice_results IS '单次练习结果详情';
COMMENT ON TABLE public.daily_reports IS '每日学习报告';
COMMENT ON TABLE public.shares IS '分享链接表';
COMMENT ON VIEW public.course_progress_view IS '课程进度统计视图';
COMMENT ON VIEW public.user_stats_view IS '用户学习统计视图';
